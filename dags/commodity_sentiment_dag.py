...
from tasks.load import load_data_to_postgres
...
with DAG(
    ...
    def scrape_news():
        ...
        return scrape_wikipedia_events(2025) 

    @task(task_id='fetch_market_prices')
    def fetch_prices():
        # Gold ('GC=F') for the year 2025
        return fetch_commodity_prices(ticker='GC=F', start_year=2025)
    ...
    @task(task_id='load_to_production_database')
    def load_data(final_dataset):
        return load_data_to_postgres(final_dataset)
    ...
    pipeline_completion = load_data(final_analyzed_data)
